...
output_dir = '../../data/processed/extracted_triples/'
...
def getTitleAbstract(dresult):
	sentences = dresult['sentences']
	title = ' '.join(dresult['sentences'][0])


	abstract = ' '.join([x for sentence in dresult['sentences'][1:] for x in sentence])
...
def extraction(filename):
	...
	print('> processing: ' + filename + ' cso reading')
	f = open(cso_dir + filename, 'r')
	paper2cso_topics = {}
	for row in f:
		drow = json.loads(row)
		paper_id = drow['doc_key']
		paper2cso_topics[paper_id] = drow
	f.close()
	...
	print('> processing: ' + filename + ' dygiepp reading for title and abstract')
	f = open(dygiepp_output_dump_dir + filename, 'r')
	paper2titleAndAbstract = {}
	for row in f:
		drow = json.loads(row)
		paper2titleAndAbstract[drow['doc_key']] = getTitleAbstract(drow)
		#print(paper2dygiepp[drow['doc_key']])
	f.close()
	...
	for paper_id in paper2cso_topics:
		if paper_id in paper2dygiepp:
			
			corenlp_out = {}
			props = {'annotators': 'openie,tokenize,pos,depparse', 'pipelineLanguage': 'en', 'outputFormat': 'json'}
			title, abstract =paper2titleAndAbstract[paper_id]
			try:
				
				text_data = title.encode('utf8', 'ignore').decode('ascii', 'ignore') + '. ' + abstract.encode('utf8', 'ignore').decode('ascii', 'ignore')
				corenlp_out = json.loads(nlp.annotate(text_data, properties=props))
				openie_triples = getOpenieTriples(corenlp_out, paper2dygiepp[paper_id], paper2cso_topics[paper_id]['semantic'] +   paper2cso_topics[paper_id]['syntactic'])
				pos_triples = getPosTriples(corenlp_out,  paper2dygiepp[paper_id], paper2cso_topics[paper_id]['semantic'] +   paper2cso_topics[paper_id]['syntactic'])
				dependency_triples = getDependencyTriples(corenlp_out,  paper2dygiepp[paper_id], paper2cso_topics[paper_id]['semantic'] + paper2cso_topics[paper_id]['syntactic'])
				entities, dygiepp_triples = manageEntitiesAndDygieepRelations(paper2dygiepp[paper_id], paper2cso_topics[paper_id]['semantic'] +   paper2cso_topics[paper_id]['syntactic'])
...
if __name__ == '__main__':
	...
	files_to_parse = [filename for filename in os.listdir(cso_dir)]
	pool = mp.Pool(4)
